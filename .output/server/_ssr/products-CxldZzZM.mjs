import { _ as prod_multiflora_default, a as honeycomb_bees_default, c as prod_bee_pollen_default, d as prod_fennel_default, f as prod_giftpack_default, g as prod_lychee_default, h as prod_luxury_hamper_default, i as honey_drizzle_default, l as prod_beeswax_candles_default, m as prod_liquid_default, n as bee_flower_default, o as prod_ajwain_default, p as prod_honeycomb_default, r as family_honey_default, s as prod_beauty_default, t as bee_farm_default, u as prod_beeswax_pellets_default, v as prod_squeeze_default, y as team_beekeepers_default } from "./team-beekeepers-DfClHD0g.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/products-CxldZzZM.js
function getProductGallery(product) {
	const list = Array.isArray(product.images) ? Array.from(new Set(product.images.filter((u) => typeof u === "string" && u.trim().length > 0))) : [];
	if (list.length > 0) return list.slice(0, 9);
	return product.image ? [product.image] : [];
}
function getProductAdditionalImages(product) {
	return (Array.isArray(product.additionalImages) ? Array.from(new Set(product.additionalImages.filter((u) => typeof u === "string" && u.trim().length > 0))) : product.attributes && typeof product.attributes === "object" && Array.isArray(product.attributes.additional_images) ? Array.from(new Set(product.attributes.additional_images.filter((u) => typeof u === "string" && u.trim().length > 0))) : []).slice(0, 8);
}
var products = [
	{
		slug: "ajwain-honey",
		...
		image: prod_ajwain_default,
		images: [
			prod_ajwain_default,
			prod_liquid_default,
			bee_farm_default,
			team_beekeepers_default
		],
		additionalImages: [
			bee_farm_default,
			honey_drizzle_default,
		...
		description: "Sourced from the nectar of Ajwain flowers, this honey is rich in antioxidants and known to support digestion, immunity and overall wellness."
	},
	{
		slug: "fennel-honey",
		name: "Fennel Honey",
		...
		description: "Collected from fennel blossoms of Saurashtra, this honey supports gut health with a subtle, refreshing flavour."
	},
	{
		slug: "lychee-honey",
		name: "Lychee Honey",
		...
		description: "Harvested from lychee orchards in bloom — smooth, mildly fruity honey that families love."
	},
	{
		...
		image: prod_multiflora_default,
		images: [
			prod_multiflora_default,
			prod_liquid_default,
			bee_farm_default,
			honeycomb_bees_default
		],
		...
		benefits: [
			"Daily wellness",
			"Balanced natural taste",
			"Rich in enzymes",
			"Everyday use"
		],
		description: "A wholesome blend from diverse wildflowers of Saurashtra — the perfect everyday jar."
	},
	{
		...
		category: "Honey",
		sizes: ["250ml", "500ml"],
		image: prod_squeeze_default,
		images: [
			prod_squeeze_default,
			prod_liquid_default,
			team_beekeepers_default,
			bee_farm_default
		],
		...
		description: "Our signature raw honey in a mess-free squeeze bottle — perfect for kitchens, tea and toast."
	},
	{
		slug: "honey-comb",
		name: "Honey Comb",
	...
	},
	{
		...
		description: "A curated trio of our best single-flora honeys in a luxury gift box — Ajwain, Fennel and Lychee."
	},
	{
		...
		description: "A healthy choice for the whole family — three larger jars of our most-loved honeys in premium packaging."
	},
	{
		...
		description: "Deeply moisturizing lip balm made with pure Saurashtra honey, natural beeswax and cold-pressed oils.",
	...
	},
	{
		...
		description: "Gentle facial scrub combining raw crystallized honey and botanical powders to buff away dull skin.",
	...
	},
	{
		...
		description: "Restorative hair treatment infused with raw honey and royal jelly to nourish dry, damaged hair.",
	...
	},
	{
		...
		description: "Rich body butter crafted with unrefined beeswax and honey to protect and soften dry skin.",
	...
	},
	{
		...
		description: "Pure aromatic beeswax blocks from our Saurashtra apiaries, ideal for DIY cosmetics, candles and salves."
	},
	{
		...
		description: "Multi-floral bee pollen granules harvested sustainably from Saurashtra wildflowers. Packed with protein and vitamins."
	},
	{
		...
		description: "Hand-rolled natural beeswax candles that burn cleanly while releasing a subtle warm honey fragrance.",
	...
	},
	{
		...
		description: "Natural beeswax condition balm formulated to protect, seal and shine fine wood and leather goods.",
	...
	},
	{
		...
		description: "Deeply moisturizing soft skin gel made with raw honey and botanical extracts for daily nourishment.",
		attributes: { Form: "Gel" }
	},
	{
		...
		image: prod_beeswax_pellets_default,
		benefits: [
			"Easy to measure & melt",
	...
	},
	{
		...
		image: prod_beeswax_pellets_default,
		benefits: [
			"Ready to roll into candles",
			"Beautiful honeycomb texture",
			"Fun for kids' crafts",
			"100% natural"
		],
	...
	},
	{
		...
		category: "Beeswax Candles",
		sizes: ["Medium", "Large"],
		image: prod_beeswax_candles_default,
		benefits: [
			"Burns longer than soy/paraffin",
	...
	},
	{
		...
		sizes: ["Standard"],
		image: prod_luxury_hamper_default,
		benefits: [
			"Exquisite wooden box",
			"4 rare single-flora honeys",
			"Includes wooden dipper",
	...
	},
	{
		...
		benefits: [
			"Complete collection",
			"Includes honey, pollen & candles",
			"Premium woven basket",
	...
	},
	{
		...
		image: prod_luxury_hamper_default,
		benefits: [
			"Festive premium packaging",
			"Variety of flavors",
			"Includes beeswax tea lights",
			"Perfect for Diwali or weddings"
		],
...
function getDefaultVariant(product) {
	...
	return vars.find((v) => v.isDefault && v.inStock !== false && (v.stock === void 0 || v.stock > 0)) || vars.find((v) => v.inStock !== false && (v.stock === void 0 || v.stock > 0)) || vars.find((v) => v.isDefault) || vars[0] || {
...
function getVariantByLabel(product, label) {
	const vars = getProductVariants(product);
	if (label) {
...
//#endregion
export { products as a, getVariantByLabel as i, getProductGallery as n, getProductVariants as r, getProductAdditionalImages as t };
