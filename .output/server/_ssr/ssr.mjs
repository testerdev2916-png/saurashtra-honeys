...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-D8KkFNSk.mjs").then((m) => m.default ?? m);
...
function applySecurityHeaders(response) {
	...
	if (contentType.includes("text/html") || contentType.includes("application/json")) headers.set("Cache-Control", "public, max-age=0, s-maxage=0, must-revalidate, no-cache, no-store");
