        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            document
              ...
              .forEach((dot) => dot.classList.remove("bg-[#3B5241]", "scale-125"));
            document
              .querySelector(`.timeline-dot-${stepNum}`)
              ?.classList.add("bg-[#3B5241]", "scale-125");
          }
        });
      },
    ...
    <section className="py-24 md:py-32 bg-[#FDFBF7] relative overflow-hidden" ref={containerRef}>
          ...
          {activeSteps.map((step, idx) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`hidden md:flex absolute left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2 w-4 h-4 rounded-full border-2 border-white bg-black/20 timeline-dot timeline-dot-${idx} transition-all duration-500 shadow-sm z-20`}
                />
                ...
                {/* Text Column */}
                <div
                  className={`w-full md:w-1/2 order-2 space-y-6 ${isEven ? "md:order-1 md:text-right" : "md:order-2 md:text-left"}`}
                >
                  <div
                    className={`flex items-center gap-4 mb-2 ${isEven ? "md:justify-end" : "md:justify-start"}`}
                  >
      ...
      <div className="container-page max-w-4xl mx-auto text-center mt-32 md:mt-48 reveal opacity-0 translate-y-8 transition-all duration-1000">
        ...
        <p className="text-[16px] md:text-[18px] text-[#2B2118]/80 leading-relaxed font-light max-w-2xl mx-auto mb-10">
          {closing_description ||
            "From a flower visited by a bee to a jar delivered to your home — that's the journey behind every jar of Saurashtra Honey."}
        </p>
        <Link
          to="/shop"
