...
async function check() {
  ...
  let { data: products, error: e1 } = await supabase.from('products').select('slug, images');
  console.log('Total products:', products ? products.length : 'error', e1);
  if (products) {
  ...
  let { data: reviews, error: e2 } = await supabase.from('reviews').select('id, media');
  console.log('Total reviews:', reviews ? reviews.length : 'error', e2);
  if (reviews) {
